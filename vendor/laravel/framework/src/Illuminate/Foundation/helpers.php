...
//     return redirect()->back()->withInput()->with([ 'login' => 'لطفا کد امنیتی را صحیح وارد کنید', ]);
// }
// <img src="{{captcha()}}" class="m-auto" width="80">
...
if (!function_exists('captcha')) {
    function captcha() {
        ...
        imagettftext($image, $font_size, 0, 15, 30, $text_color, 'asset/front/fonts/Vazir-Bold.ttf', $string);
...
if (!function_exists('set_meta')) {
    function set_meta($model, $item_id, $name_page, $description, $keyword, $title_page, $priority, $schema) {
        Meta::create([
            'model'         => $model,
            'item_id'       => $item_id,
            'name_page'     => $name_page,
...
if (!function_exists('update_meta')) {
    function update_meta($id, $name_page, $description, $keyword, $title_page, $priority, $schema) {
        $meta = Meta::find($id);
        $meta->name_page    = $name_page;
        $meta->description  = $description;
        $meta->keyword      = $keyword;
        $meta->title_page   = $title_page;
...
if (!function_exists('set_lang')) {
    function set_lang($item, $fild, $lang = null)
    {
        if ($lang == null) {
        ...
        if ($lang == 'fa') {
            ...
            if (count($item->langs)) {
                foreach ($item->langs as $item_lang) {
                    if ($item_lang->fild_name == $fild) {
                        $fild_name = 'text_' . $lang;
                        return $item_lang->$fild_name;
...
if (!function_exists('store_lang')) {
    function store_lang($request, $type, $item_id, $filds)
    {
...
if (!function_exists('check_lang')) {
    function check_lang($item, $type, $lang = null)
    {
        if ($lang == null) {
            $lang = app()->getLocale();
        }
        if ($lang == 'fa') {
            ...
            $langs = Lang::where('type', $type)->where('item_id', $item->id)->get();
            if (count($langs) > 0) {
                return true;
            } else {
                return false;
...
if (!function_exists('user_like')) {
    function user_like($id, $model)
    {
        $user_like_id = '';
        if (Auth::check()) {
            $user_like_id = Auth::user()->id;
        } else {
            if (empty(session('user_like_id'))) {
                $code = mt_rand(11111, 99999);
                session(['user_like_id' => $code]);
                $user_like_id = $code;
            } else {
                $user_like_id = session('user_like_id');
            }
        }
        $user_like = \App\UserLike::where('user_id', $user_like_id)->where('model', $model)->where('model_id', $id)->first();
...
if (!function_exists('asset')) {
    ...
    {
        return app('url')->asset('/includes/asset/' . $path, $secure);
    }
}
