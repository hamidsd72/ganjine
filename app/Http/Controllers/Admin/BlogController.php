...
//        $this->middleware(['auth', 'clearance']);
    ...
    public function store(Request $request,$type) {
        ...
        try {
            ...
            if ($request->hasFile('photo')) {
                $photo = new Photo();
                $photo->path = file_store($request->photo, 'asset/uploads/blogs/'.$type.'/photos/', 'blog-photo-');
                $item->photo()->save($photo);
            }
            
            $model          = 'App\Models\Blog';
            $item_id        = $item->id;
            $name_page      = url('/').'/blog/show/'.$request->slug;
    ...
    {
        $item = Blog::find($id);
        return view('admin.blog.edit', compact('item'), ['title' => $item->title]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('photo')) {
                ...
                $photo = new Photo();
                $photo->path = file_store($request->photo, 'asset/uploads/blogs/'.$item->type.'/photos/', 'blog-');
                $item->photo()->save($photo);
            }
            $item->save();
            ...
            $item->activity()->save($activity);
            return redirect('admin/blog-list/'.$item->type)->with(['status' => 'success', "message" => ' با موفقیت ویرایش شد.']);
        } catch (\Exception $e) {
            return redirect()->back()->with(['status' => 'danger-600', "message" => 'یک خطا رخ داده است، لطفا بررسی بفرمایید.']);
        }
    }
