...
class SliderController extends Controller {
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            $item->url_name             = $request->url_name;
            $item->url2_name            = $request->url2_name;
            $item->url                  = mb_convert_encoding($request->url, 'HTML-ENTITIES', "UTF-8");
            $item->url2                 = mb_convert_encoding($request->url2, 'HTML-ENTITIES', "UTF-8");
            $item->url_color            = $request->url_color;
            $item->url_background       = $request->url_background;
            $item->url_hover_color      = $request->url_hover_color;
            $item->url_hover_background = $request->url_hover_background;
            if ($request->hasFile('img_top')) {
                $item->img_top = file_store($request->img_top, 'asset/uploads/sliders/photos/', 'slider-top-');
            }
            $item->save();

            if ($request->hasFile('photo')) {
                $photo = new Photo();
                $photo->path = file_store($request->photo, 'asset/uploads/sliders/photos/', 'slider-photo-');
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('photo')) {
                ...
                $photo = new Photo();
                $photo->path = file_store($request->photo, 'asset/uploads/sliders/photos/', 'slider-');
                $item->photo()->save($photo);
            }
            $item->save();
