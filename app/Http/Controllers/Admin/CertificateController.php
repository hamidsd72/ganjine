...
class CertificateController extends Controller {
    ...
    public function store_certificate(Request $request) {
        ...
        try {
            if ($request->hasFile('pic')) {
                $pic = file_store($request->pic, 'asset/img/bc_cv/', 'C-');
                $pic='/'.$pic;
            }
            $cer        = new Certificate();
            $cer->pic   = $pic;
    ...
    public function del_certificate($id) {
        ...
        try {
            ...
            $old_t  = $cer->title;
            if (is_file($cer->pic)) File::delete($cer->pic);
            $cer->delete();
            $activity = new Activity();
            $activity->user_id = Auth::user()->id;
    ...
    public function update_certificate(Request $request, $id) {
        ...
        try {
            ...
            if ($request->hasFile('pic')) {
                if(is_file($cer->pic)) File::delete($cer->pic);
                $pic = file_store($request->pic, 'asset/img/pharmacy/map/', 'map-');
                $pic = '/'.$pic;
                $cer->pic =$pic;
            }
            ...
            if($cer->langs) foreach ($cer->langs as $lang) $lang->delete();
