...
class HomeController extends Controller {
    ...
    public function excel(Request $request) {
        ...
        if ($request->hasFile('excel')) {
            $excel = new Photo();
            $excel->path = file_store($request->excel, 'asset/uploads/product/excel/', 'excel-');
            $url = $excel->path;
        }
    ...
    public function index() {
        $activities = Activity::orderByDesc('id')->take(5)->get();
    ...
    public function upload_store(Request $request) {
        ...
        try {
            if ($request->hasFile('file')) {
                ...
                $file->link = file_store($request->file, 'asset/uploads/files/', 'file-');
                $file->save();
    ...
    public function upload_del($id) {
        $file = Upload::find($id);
        try {
            if (is_file($file->link)) File::delete($file->link);
            $file->delete();
    ...
    public function visit() {
        ...
        //visit week
        $week = VisitLog::whereBetween('created_at', [Carbon::today()->startOfWeek(), Carbon::today()->endOfWeek()])->count();
        //visit week
        $month = VisitLog::whereBetween('created_at', [Carbon::today()->startOfMonth(), Carbon::today()->endOfMonth()])->count();
        //visit all
    ...
    public function activities() {
        $items = Activity::orderByDesc('id')->get();
    ...
    public function serachCode(Request $request) {
        ...
        $code = Code::where('code', 'LIKE', '%' . $request->text . '%')->paginate($this->controller_paginate());
        return view('admin.excel.index', compact('code'));
    }

    public function email() {
        $items = Email::paginate($this->controller_paginate());
